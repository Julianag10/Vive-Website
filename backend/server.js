...
// 1. express-session TRIES reads cookie from req
// 2a. FIRST ITEM LOGINS:
// - no cookie = cookie was never sent -> session was never saved to RAM
// - req.session:
//      - creates (DOSEN'T SAVE to RAM YET)new empty session
//      - saves cookie to res, but dosent send 
// 
...
// 3. IF cookie exists uses the sessionID to find session in RAM 
// 4. req.session ATTACHES the found session from RAM to req
// 5. No SESSIONID is saved to store(RAM) & NO COOKIE is sent UNTILL somthing to req.session(bc not need to resave unchanged sessions)
app.use(
  session({
    // cookie name 
    ...
    //  dont save sessions unless something is initalized/reintialized(wirte something to req.session: req.session.admin = admin)
...
// Public
app.use("/api/checkout", checkoutRouter); // reutens pure JSON
