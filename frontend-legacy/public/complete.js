...
function setSessionDetails(session) {
    ...
    switch(session.status){
        case "complete":
            ...
            iconColor = "#30B130"; // green
            icon = SuccessIcon;
            break;
        case "open":
            statusText ="Payment Failed";
            iconColor = "#DF1B41"; // red
            icon = ErrorIcon;
            break;
        default:
            break;
    ...
    // overall checkout session status
    // did chekcout flow finish or not
    // open, complete(finished oaying and reache success url), expired
    ...
    // session.status → “Did the session complete?”
    // session.payment_status → “Did the payment itself succeed?”
    // session.payment_intent_status → “What’s the detailed Stripe PaymentIntent status?”

    // dynamivally sets the view details link in in HTML in stripe dahs board
    document.querySelector("#view-details").href = 
        `https://dashboard.stripe.com/payments/${session.payment_intent_id}`;
}

// only used when the session dose not exists
...
async function initialize(){
    // grabs the fill string from the current page URL
    const queryString = window.location.search;

    // turns the string into an easy to read object
    const urlParams = new URLSearchParams(queryString);

    // extracts th values of the session_id paramenrere
