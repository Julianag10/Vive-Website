...
// i feel like te adminporgrmas page is not good porgraming the component is too long 
// and dose to many things i know that at least saveign to db shoudl be a sepreate component
// maybe admin program page shoudl just be in chage of calling/rendind all the compoents

// TODO Add form control (checkbox/toggle) bound to form.is_featured.
// TODO Optionally add dedicated feature-toggle endpoint Separate “activate/deactivate” from “feature/unfeature” for cleaner admin actions.
// TODO add program image upload? + Store an image URL/path in DB
// TODO change progrma image
// TODO change any aspect of a program (title, description, dates, capacity, is_featured) from edit form (currently only title and capacity can be edited from edit form)
// TODO Add button in AdminPrograms.jsx to call toggle route: toggleProgramFeatured(id, isFeatured) and update UI accordingly.
export default function AdminPrograms() {
  // progrmas state is array of programs
  // bc page displays all progrmas, fetchPrograms()
  // TODO  introduce boolean handling in admin UI state (form, editForm, setForm, setEditForm).
  //
  ...
  useEffect(() => {
    ...
    // TODO UNDESTAND
    // WHY not useEffect(() => { ... }) (no deps), run after every render 
  }, []); // [] -> run once on mount not automtically when somethgni changes, bc progrmas run on load so notohign to change when first rendered
  ...
  // TODO UNDESTAND
  // fethc is. aside effect but triggerd after a redner dinishes
  ...
  // TODO add toggle featured fucntions
  ...
  async function handleCreateProgram(e) {
    ...
    try {
      const res = await fetch("/admin/api/programs", {
        ...
        body: JSON.stringify({
          // TODO UNDESRATND
          // TODO Include is_featured in POST body for create.
  ...
  async function saveEdit(programId) {
    try {
      const res = await fetch(`/admin/api/programs/${programId}`, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        // TODO Include is_featured in edit form + PUT body for updates.
